...
use crate::{
    utils::{
        context::Ext,
        spotify::{self, get_access_token},
        ytmusic::{self, search::parsing::SearchResult},
    },
    CLIENT,
};

async fn spotify_request_token_and_save(
    data: Option<&mut Option<spotify::AccessToken>>,
) -> Result<spotify::AccessToken, &'static str> {
...
pub(super) async fn extract_spotify(
    ctx: &Context,
    msg: &Message,
    spotify_url: &str,
    ...
    let token = data
        ...
        .expect("should at least be None");

    let token = if let Some(token) = token {
        info!("got spotify token from cache");
        token
    } else {
        &spotify_request_token_and_save(data.get_mut::<spotify::AccessToken>()).await?
    };
    ...
    let Ok(spotify_track) = spotify::find_track_from_url(spotify_url, token).await else {
