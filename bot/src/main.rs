...
/// The global registered commands.
pub static COMMANDS: LazyLock<Vec<&'static BotCommand>> =
    LazyLock::new(|| inventory::iter::<BotCommand>.into_iter().collect());
...
async fn main() -> Result<()> {
    ...
    let cmds = COMMANDS.len();
    info!("found {cmds} cmds!");
