...
use crate::COMMANDS;
use utils::{context::CtxExt, reply::Replyer, Arg, Args};
...
impl EventHandler for SlashCommands {
    ...
    async fn ready(&self, ctx: Context, _: Ready) {
        // we use `inventory` to find commands so we don't have to do it manually.
        let commands = COMMANDS.iter().map(|c| (c.register)());

        let testing_guild = GuildId::new(TESTING_GUILD);
        if cfg!(debug_assertions) {
            testing_guild
                .set_commands(&ctx.http, commands.collect())
                .await
