...
/// # Errors
///
/// Fails if request could not be sent, or did not succeed.
async fn get_base(client: &Client) -> anyhow::Result<Response> {
    let resp = client
...
/// # Errors
///
/// Will fail if cfg blob could not be found, or `VISITOR_DATA` is missing, or it could not be parsed as a string.
fn parse_visitor_id(resp: &str) -> anyhow::Result<String> {
    ...
    let visitor_id = cfg
        .get("VISITOR_DATA")
        .ok_or(anyhow!("failed to find VISITOR_DATA from cfg"))?;
...
/// # Errors
///
/// Errors if http requests fail, or if `visitor_id` could not be parsed.
/// 
/// # Panics
/// 
/// Will panic if a `set-cookie` header could not be parsed. 
pub async fn search<S: AsRef<str>>(
    ...
    let base_resp = get_base(client).await?;
    ...
    let base_resp = base_resp.text().await?;

    let visitor_id = VISITOR_ID
        .get_or_try_init(async || parse_visitor_id(&base_resp))
        .await?;
