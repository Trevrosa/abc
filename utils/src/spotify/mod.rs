...
/// # Errors
/// 
/// - If the spotify token could not be requested
/// - If the track metadata could not be fetched from spotify api
/// - If the youtube api key could not be requested, or refreshed
/// - If the user chooses to cancel the choice.
/// - If the user does not make a choice.
/// 
/// # Panics
/// 
/// Will panic if we fail to create a response to an interaction.
pub async fn extract_spotify(
    ...
    let token = if let Some(token) = token {
        ...
        &spotify_request_token_and_save(client, data.get_mut::<AccessToken>()).await?
    };
    ...
    let Ok(spotify_track) = find_track_from_url(client, spotify_url, token).await else {
    ...
    let stored = data.get::<ytmusic::AccessToken>().expect("should be at least None");
    ...
    let access_token: ytmusic::AccessToken = match stored {
        Some(token) => {
            if token.expired() {
                ...
                if let Err(err) = token.refresh(client).await {
        ...
        None => {
            let auth = ytmusic::oauth(ctx, replyer, client).await;
    ...
    let searched = ytmusic::search(client, query.as_str(), access_token.as_ref()).await;
