...
use serenity::all::{
    ...
    CreateButton, CreateEmbed, CreateInteractionResponseFollowup, CreateMessage, CreatePoll,
    Message, create_poll,
};
...
impl Replyer<'_> {
    /// Try to coerce `&self` to a [`Message`], panicking if `&self` was actually [`Replyer::Slash`].
    ...
    /// Try to coerce `&self` to a [`CommandInteraction`], panicking if `&self` was actually [`Replyer::Prefix`].
...
pub trait IntExt {
    ...
    ) -> impl Future<Output = Result<Message, serenity::Error>>;
}
